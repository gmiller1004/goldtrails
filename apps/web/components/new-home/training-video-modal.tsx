...
const YOUTUBE_VIDEO_ID = "P5fLiR2eM0E";

type TrainingVideoModalProps = {
  className?: string;
};

export function TrainingVideoModal({ className }: TrainingVideoModalProps) {
  ...
  return (
    <>
      <button
        ...
        className={cn(nhGoldButtonClass, "mt-6 gap-2 px-6 py-3 text-sm", className)}
      >
        <Play className="h-4 w-4 shrink-0 fill-current" aria-hidden />
        Watch a Sample of Kevin&apos;s Training
      </button>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between border-b border-[#5c4f3f]/40 px-4 py-3 sm:px-5">
              <p id="training-video-title" className="text-sm font-semibold text-[#f7f2e8] sm:text-base">
                Sample training with Kevin Hoagland
              </p>
              <button
                type="button"
                onClick={() => setOpen(false)}
            ...
            <div className="relative aspect-video w-full bg-black">
              <iframe
                ...
                title="Kevin Hoagland training sample"
